...
void GPIOInit() {
	...
	LPC_GPIO0->DIR |= (0x1<<LED_BIT);	//enable GPIO pin7 as output (led)
...
void TIMER32_0_IRQHandler(){
	...
	if (timerCount > 10000){
		tenSecondToggle = 1 - tenSecondToggle;
		timerCount = 0;
		startTime = 0;
		stopTime = 0;
	}
	if (firstRead) {
		...
		if(tenSecondToggle)    // before 10 seconds
		{
			if (ledStatus == OFF) {
				if (ledTimer == 0) {
					ledTimer = led25Percent;
					//turn on led
					LPC_GPIO0->MASKED_ACCESS[(1<<LED_BIT)] = (ON<<LED_BIT);
					ledStatus = ON;
				}
				else {
					ledTimer--;
				...
				if (ledTimer == 0) {
					ledTimer = led25Percent;
					//turn off led
					LPC_GPIO0->MASKED_ACCESS[(1<<LED_BIT)] = (OFF<<LED_BIT);
					ledStatus = OFF;
				}
				else {
				ledTimer--;
		...
		if(!tenSecondToggle)    // after 10 seconds
		{
			if (ledStatus == OFF) {
				if (ledTimer == 0) {
